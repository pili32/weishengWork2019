  ...
  <div style="height:100%;position:relative;">
    ...
    >
      ...
      <Form :model="editObje" ref="editValidate" :label-width="80" :rules="editValidate">
        ...
        <FormItem label="关键字" prop="keyworks">
          <Input v-model="editObje.keyworks" placeholder="请输入关键字" />
        </FormItem>
        <FormItem label="来源" prop="sourceName">
          <Input v-model="editObje.sourceName" placeholder="请输入来源" />
        </FormItem>
        <FormItem label="来源地址" prop="sourceAddress">
        ...
        <FormItem label="内容" prop="content">
          <editor
            ref="editor"
            v-model="editObje.content"
            :value="editObje.content"
        ...
        <div style="padding-left:10px;">
          ...
          >
            <Button icon="ios-cloud-upload-outline" style="display:inline-block">选择文件</Button>
          </Upload>
          <span style="color:blue;margin-left:80px">{{fileName}}</span>
        </div>
      </Form>
      <div slot="footer">
    ...
    <Modal title="查看内容" v-model="LookModal" :footer-hide="true" :mask-closable="false" draggable>
      <Form :label-width="85" v-if="lookObject" style="margin-bottom: 20px;">
        ...
        <FormItem label="内容：">
          <p class="chaKan" v-html="lookObject.content"></p>
        </FormItem>
      </Form>
    </Modal>
  </div>
  ...
  methods: {
    ...
    handleSuccess1(res, file) {
      if (res.code == 10000) {
        ...
        let file = res.datas[0].relativePath;
    ...
    handleChange(html, text) {
      if (html) this.editObje.content = html;
    },
    look(row) {
      this.lookObject = {
        newsType: row.newsType,
