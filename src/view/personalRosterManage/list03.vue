      ...
      <Form inline :model="queryPar">
        ...
        <Row>
          <FormItem>
            ...
            <Button type="primary" @click="onEmpty()">清空所有预传在校生名单</Button>
          </FormItem>
          <FormItem>
            <Upload
              ref="upload"
              ...
              :format="['xlsx','xls']"
            >
              <Button type="primary" icon="ios-cloud-upload-outline">预传在校生名单导入</Button>
            </Upload>
          </FormItem>
          <FormItem>
            <span>
...
export default {
  ...
  created() {
    ...
    this.uploadUrl =
      process.env.baseUrl + "/hbwsrc-infoplatform/" + "studentRoster/importAll";
  },
  mounted() {
    //计算table的高度
    this.$nextTick(() => {
  ...
  methods: {
    reload(page) {
      ...
      //统计
      studentRosterStatistics().then(res => {
        if (res.code == 10000) {
          let d = res.datas;
    ...
    download() {
      Modal.confirm({
        title: "提示信息",
        content: "确认下载【预传在校生人员名单模板】？",
        onOk: () => {
          let fileUrl = "../../../static/files/预传在校生人员名单模板.xls"; // 文件地址
          let downName = "预传在校生人员名单模板.xls"; // 文件下载名称
    ...
    onDel(id) {
      Modal.confirm({
        title: "提示信息",
        content: "删除后不影响在校生名册名册结果，确认删除？",
        onOk: () => {
          let par = {
            id: id
          };
          deletes(par).then(res => {
            if (res.code == 10000) {
              Message.success("操作成功");
              this.showDtlsModal = false;
              this.reload();
    ...
    onEmpty() {
      Modal.confirm({
        title: "提示信息",
        content: "清空所有预传在校生名单不影响在校生名册名册结果，确认清空？",
        onOk: () => {
          deleteAll().then(res => {
            if (res.code == 10000) {
              Message.success("操作成功");
